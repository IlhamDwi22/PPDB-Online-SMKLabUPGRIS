...
@section('title', 'Step 6 — Review & Finalisasi')

@section('content')
    <x-stepper :currentStep="6" />
    ...
    <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-800">Review & Finalisasi</h2>
        <p class="text-sm text-gray-500 mt-1">Periksa kembali semua data sebelum melakukan finalisasi</p>
    </div>
    ...
    {{-- Action Buttons --}}
    @if(!($student->is_finalized ?? false))
        <div class="flex justify-between items-center p-5 sm:p-6 bg-white rounded-xl shadow-sm border border-gray-100">
            <x-button variant="secondary" :href="url('/student/form/step/5')" type="button">
            ...
            <form method="POST" action="{{ url('/student/finalize') }}">
                @csrf
                <x-button variant="accent" onclick="return confirm('Apakah kamu yakin ingin memfinalisasi data? Data yang sudah difinalisasi tidak bisa diubah lagi.')">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
